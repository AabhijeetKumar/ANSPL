...
import { projectConsulting} from "./projectConsulting";
import { motion } from "framer-motion";
import { FiArrowRight } from "react-icons/fi";
import { useNavigate } from "react-router-dom";

export default function ProjectConsultingItems() {
  const navigate = useNavigate();

  const { productId } = useParams();
  const product = projectConsulting.find((p) => p.id === productId);

  if (!product)
    return <h2 className="p-10 text-center text-white">Service Not Found</h2>;
